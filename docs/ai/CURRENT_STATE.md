...
- В разделе `Слова` визуалы слов задаются через `WordVisual`: неоднозначные слова `desk`, `board`, `coat`, `jacket` рисуются отдельными пиктограммами, чтобы не показывать стул вместо парты или носки вместо верхней одежды. Регрессия покрыта `frontend/src/subjects/english/data/vocabulary.test.ts`.
...
- Рейтинги реальных пользователей синхронизируются через backend: таблица `leaderboard_score`, `POST /api/leaderboard/score`, `GET /api/leaderboard?subject=...`. Локальные моковые списки используются только как offline/dev fallback.
- На hub есть share приложения через Web Share API с clipboard fallback.
- Нижнее меню предметов приведено к общему порядку `Главная / Учёба / Задания / Рейтинг / Профиль`.
- Заблокированные достижения должны показывать условие получения, а не только статус блокировки.
